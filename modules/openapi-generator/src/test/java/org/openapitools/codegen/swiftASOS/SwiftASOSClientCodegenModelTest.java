...
public class SwiftASOSClientCodegenModelTest {
    ...
    @Test(description = "convert a model with USE_CUSTOM_DATE_WITHOUT_TIME enabled")
    ...
    @Test(description = "ASOS naming: Asos→ASOS is applied to model property names")
    public void asosNamingInModelPropertiesTest() {
        final Schema schema = new Schema()
                .addProperty("asosId", new StringSchema())
                .addProperty("asosUrl", new StringSchema());
        ...
        Assert.assertEquals(cm.vars.get(0).name, "ASOSID");
        Assert.assertEquals(cm.vars.get(1).name, "ASOSURL");
    }
}
